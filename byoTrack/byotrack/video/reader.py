...
class TiffSequenceReader(VideoReader):
    """
    Video reader for files saves as a sequence of individual tiff images
    Uses skimage backend (calling PIL)
    """
    ...
    def __init__(self, path: Union[str, os.PathLike], **kwargs):
        super().__init__(path, **kwargs)
        vid = iio.ImageCollection(path + '/*.tif').concatenate()
        self.video = vid.reshape(vid.shape[0], vid.shape[1], vid.shape[2], 1)
        self.shape = self.video[0].shape
    ...
    def retrieve(self) -> np.ndarray:
        frame = self.video
        return frame  # In PIL, RGB images are read as RGB. Let's flip it to BGR
